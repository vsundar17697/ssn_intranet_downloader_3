...
parser.add_argument("-b", "--branch", help="Branch ID. 5 for CSE. 9 for IT", type=str)
parser.add_argument("-s", "--sem", help="Semester", type=int)
...
global sem , year , branch 
sem = 0
year = 0
branch = 0

def processYearID(sem_val):
    year_id = ((sem_val - 1)/2)
    return (int)(year_id)
...
def processSemVal():
    if args.sem:
        ...
        sem_val = (int)(input('Enter Semester >> '))
        args.sem = sem_val
        return sem_val

def processSemID(sem_val):
    sem_id = sem_val % 2
    sem_id = sem_id if sem_id == 1 else 2
    return sem_id
...
def processBranchID():
    ...
    if args.branch:
        ...
        br = (str)(input('Enter branch (CSE / IT ONLY ) >>'))
        args.branch = br
    br = br.lower()
...
def DownloadFiles(file_refs, folderpath, types):
    ...
    for i in file_refs:
        ...
        sys.stdout.write('\r')
        sys.stdout.flush()
        sys.stdout.write("Progress : [%-50s] %d%%" % ('=' * (int)(percentage / 2), percentage))
...
def getVal(sites, message, value=0):
    if value == 0:
...
def getSubURL(mainurl):
    ...
    subject_id = getVal(all_sites, 'Subject >> ', branch)
...
def createPrimeHTML(subfiles,path):
    ...
    html = (str)(urllib.request.urlopen(prime_url).read())
        ...
        for rawlink in name[section]:
            ...
            try:
                for i in replacements:
                    html = html.replace((str)(i), 'file://' + replacements[i])
                html = html.replace('\\n' , '')
...
if __name__ == '__main__':
    sem_val = processSemVal()
    sem = processSemID(sem_val)
    year = processYearID(sem_val)
    branch = processBranchID()
    try:
        main()
    except:
        print('Ran into some error , please check if you are connected to ssn and verify that proxy is disabled and try again.')
    del sem , year , branch
